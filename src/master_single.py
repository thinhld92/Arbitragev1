...
VALID_TRADE_MODES = {"hedge", "single"}
...
def normalize_mode(cap):
    ...
    if mode not in VALID_TRADE_MODES:
        raise ValueError(f"trade_mode khong hop le: {mode}")
    return mode
...
def resolve_execution(cap):
    ...
    if mode != "single":
        raise ValueError("master_single.py chi phuc vu trade_mode='single'")
    ...
    if exec_key == base_key:
        role = "BASE"
    elif exec_key == diff_key:
        role = "DIFF"
    else:
        raise ValueError(
            "execution.exchange/symbol phai trung voi base_exchange/base_symbol "
            "hoac diff_exchange/diff_symbol"
        )

    default_volume = cap.get("volume_base", 0.01) if role == "BASE" else cap.get("volume_diff", 0.01)
...
def infer_loai_lenh_from_side(execution_role, side):
    side = safe_upper(side)
    if execution_role == "BASE":
        if side == "SELL":
            return "TH1"
        if side == "BUY":
            return "TH2"
    elif execution_role == "DIFF":
        if side == "BUY":
            return "TH1"
        if side == "SELL":
            return "TH2"
...
def infer_open_action(execution_role, tin_hieu):
    if execution_role == "BASE":
        return tin_hieu.get("lenh_base")
...
def make_context(cap, execution, order_data, close_data):
    return {
        ...
        "action_type": close_data.get("action_type", "SINGLE_CLOSE"),
...
log_filename = os.path.join(log_dir, f"log_master_single_{args.pair_id}.txt")
logging.basicConfig(
    level=logging.INFO,
...
try:
    ...
    print(f"LOI config single: {exc}")
    logging.error("LOI config single: %s", exc)
    raise SystemExit(1)

vps_name = config.get("vps_name", "LOCAL")
master_name = f"[{vps_name} | SINGLE | {args.pair_id}]"
try:
    ctypes.windll.kernel32.SetConsoleTitleW(f"MASTER SINGLE {master_name}")
except Exception:
    pass
dan_tran_cua_so(4)
...
if saved_state_raw:
    ...
    print(f"[SINGLE] Khoi phuc so lenh: {len(lich_su_lenh)} lenh.")
else:
    huong_dang_danh = None
    lich_su_lenh = []
    thoi_diem_vao_lenh_cuoi = 0
    thoi_diem_vua_ra_lenh_dong = 0
    print("[SINGLE] Bat dau voi so lenh trong.")
...
def reload_runtime_config(new_cap):
    ...
    if old_exec_key != new_exec_key and len(lich_su_lenh) > 0:
        print("[HOT RELOAD] Dang co lenh single, khong doi execution broker/symbol.")
...
print(
    f"[SINGLE] Master san sang: execution={execution['exchange']}:{execution['symbol']} "
...
try:
        ...
        try:
            ...
            if now_sec - last_config_check_time >= 1.0:
                ...
                if r.get(SHUTDOWN_KEY):
                    print("[SHUTDOWN] Master single nhan tin hieu tat.")
                ...
                if current_modified != last_config_modified:
                    ...
                    try:
                        ...
                        if new_cap:
                            ...
                            logging.info(
                                "[HOT RELOAD SINGLE] entry=%s close=%s pivot=%s execution=%s:%s",
                                dev_entry,
                                dev_close,
                                spread_pivot,
                                execution["exchange"],
            ...
            pos_raw, tick_base_raw, tick_diff_raw, eq_raw = r.mget(
                execution["position_key"], key_base, key_diff, execution["equity_key"]
            )
            ...
            while True:
                ...
                try:
                    ...
                    loai_lenh = draft.get("loai_lenh") or infer_loai_lenh_from_side(execution["role"], action)
                    order_data = {
                        "id_lenh": f"SINGLE_{job_id}",
                        "ticket": ticket,
                        "role": execution["role"],
                        "action": action,
                        "loai_lenh": loai_lenh,
                    ...
                    luu_tri_nho()
                    print(f"[SINGLE] Da ghi so ticket #{ticket} {action} ({loai_lenh}) tu job {job_id}.")
                except Exception as exc:
                    print(f"[SINGLE] Loi doc ORDER_RESULT: {exc}")

            expired_jobs = [jid for jid, data in pending_jobs.items() if now_sec - data.get("time", now_sec) > 60]
            for jid in expired_jobs:
                print(f"[SINGLE] Xoa job mo lenh qua 60s khong co ticket: {jid}")
            ...
            if not trong_thoi_gian_bao_ve:
                ...
                for order_data in lich_su_lenh:
                    ...
                    print(f"[SINGLE] Ticket #{order_data['ticket']} bien mat. Lay lich su va ghi so.")
                ...
                if untracked_positions and not trong_thoi_gian_khoi_dong:
                    for pos in sorted(untracked_positions, key=lambda x: x.get("time_msc", 0)):
                        ...
                        loai_lenh_pos = infer_loai_lenh_from_side(execution["role"], side)
                        if loai_lenh_pos == "UNKNOWN":
                            ...
                            print(f"[SINGLE] Cat lenh khong ro huong #{pos['ticket']} bang ticket.")
                            thoi_diem_vua_ra_lenh_dong = time.time()
                            continue

                        if huong_dang_danh and loai_lenh_pos != "UNKNOWN" and loai_lenh_pos != huong_dang_danh:
                            order_data = {
                                "id_lenh": f"ORPHAN_SINGLE_{pos['ticket']}",
                                "ticket": pos["ticket"],
                                "role": execution["role"],
                                "action": side,
                            ...
                            gui_lenh_dong(order_data, close_data, "ORPHAN_SINGLE")
                            print(f"[SINGLE] Cat lenh la nguoc huong #{pos['ticket']} {side}.")
                            thoi_diem_vua_ra_lenh_dong = time.time()
                        else:
                            adopted = {
                                "id_lenh": f"ADOPT_SINGLE_{pos['ticket']}",
                                "ticket": pos["ticket"],
                                "role": execution["role"],
                                "action": side,
                                "loai_lenh": loai_lenh_pos,
                            ...
                            print(f"[SINGLE] Adopt ticket #{pos['ticket']} {side} ({loai_lenh_pos}) vao so lenh.")
                    luu_tri_nho()
            ...
            if not tick_base.get("connected", False) or not tick_diff.get("connected", False):
                print("[SINGLE] Mat mang/tick source, khoa lenh.", end="\r")
                thoi_diem_nhan_tick_cuoi = time.time()
            ...
            if now - local_nhan_base > max_tick_delay or now - local_nhan_diff > max_tick_delay:
                print(f"[SINGLE] Tick tre qua {max_tick_delay}s, khoa lenh.", end="\r")
                da_xu_ly_lenh_cho_tick_nay = True
                continue

            if trong_gio_cam:
                if lich_su_lenh:
                    print(f"[SINGLE] Gio cam: xa {len(lich_su_lenh)} lenh single.")
                    for order_data in lich_su_lenh[:]:
                        close_data = {
                            "chenh_dong": 0,
                            "chenh_dong_raw": 0,
            ...
            if hanh_dong == "DONG_LENH" and lich_su_lenh:
                if tick_hz_vuot_nguong:
                    print(f"[SINGLE] Tick Hz vuot nguong, khoa chot. {hz_base}|{hz_diff}", end="\r")
                    continue
                ...
                skip_close, reason_close = should_skip_by_filter(
                    filter_close, close_action, gia_base_luc_bat_dau_lech_dong, tick_base["bid"]
                )
                if skip_close:
                    print(f"[SINGLE] Huy dong lenh: {reason_close}")
                    thoi_diem_bat_dau_lech_dong = 0
                    gia_base_luc_bat_dau_lech_dong = 0.0
                    continue
                ...
                close_data = {
                    ...
                    "mode_dong": mode_dong,
                    "action_type": tin_hieu.get("loai_dong", "SINGLE_CLOSE"),
                    "conf_dev_close": dev_close,
                    "tick_hz_base_out": tick_base.get("tick_hz", 0),
                    "tick_hz_diff_out": tick_diff.get("tick_hz", 0),
                }
                ...
                luu_tri_nho()
                print(
                    f"[SINGLE] Dong ticket #{order_data['ticket']} {close_action}, "
                    f"dev {close_data['chenh_dong']:.2f}."
                )

            elif hanh_dong == "VAO_LENH":
                if tick_hz_vuot_nguong:
                    print(f"[SINGLE] Tick Hz vuot nguong, khoa vao. {hz_base}|{hz_diff}", end="\r")
                    continue
                if equity_execution < alert_equity:
                    print(
                        f"[SINGLE] LOW EQUITY {execution['exchange']} {equity_execution:.2f} < {alert_equity}",
                        end="\r",
                    )
                    continue
                ...
                action = infer_open_action(execution["role"], tin_hieu)
                skip_entry, reason_entry = should_skip_by_filter(
                    filter_entry, action, gia_base_luc_bat_dau_lech, tick_base["bid"]
                )
                if skip_entry:
                    print(f"[SINGLE] Huy vao lenh {loai_lenh_moi}: {reason_entry}")
                    thoi_diem_bat_dau_lech_vao = 0
                    gia_base_luc_bat_dau_lech = 0.0
                    continue
                ...
                luu_tri_nho()
                print(
                    f"[SINGLE] Vao {loai_lenh_moi} {action} {execution['exchange']}:{execution['symbol']} "
                    f"dev {tin_hieu['chenh_lech']:.2f} job {job_id}."
                )
                logging.info(
                    "[SINGLE ENTRY] %s %s dev=%s raw=%s job=%s",
                    loai_lenh_moi,
                    action,
                    tin_hieu["chenh_lech"],
            ...
            print(f"\n[SINGLE CRITICAL] {exc}")
            logging.error("[SINGLE CRITICAL]", exc_info=True)
            thoi_diem_nhan_tick_cuoi = time.time()
            da_xu_ly_lenh_cho_tick_nay = True
            time.sleep(0.5)
            continue
    ...
    print(f"\n[MASTER SINGLE {args.pair_id}] Tat an toan.")
    logging.info("=== STOP MASTER SINGLE ===")

print(f"[MASTER SINGLE {args.pair_id}] Da thoat.")
