...
def normalize_mode(cap):
    ...
    if mode not in VALID_TRADE_MODES:
        raise ValueError(f"master_copy_base chi phuc vu trade_mode='copy_base', nhan: {mode}")
    return mode


def resolve_execution(cap):
    """
    Phan giai block execution cho COPY_BASE.
    Khac voi single: san execution KHONG CAN trung base/diff, va role luon la COPY_BASE.
    """
    ...
    # copy_base: role luon la COPY_BASE, logic vao lenh theo chieu cua Base
    default_volume = cap.get("volume_base", 0.01)
...
def infer_loai_lenh_from_side(side):
    """
    Copy_base luon theo chieu Base:
    - TH1: SELL Base -> SELL san 3
    - TH2: BUY Base -> BUY san 3
    """
...
def infer_open_action(tin_hieu):
    """Copy_base luon lay lenh tu chieu Base."""
...
def make_context(cap, execution, order_data, close_data):
    return {
        ...
        "action_type": close_data.get("action_type", "COPY_BASE_CLOSE"),
...
log_filename = os.path.join(log_dir, f"log_master_COPY_BASE_{args.pair_id}.txt")
logging.basicConfig(
    level=logging.INFO,
...
try:
    ...
    print(f"LOI config COPY_BASE: {exc}")
    logging.error("LOI config COPY_BASE: %s", exc)
    raise SystemExit(1)

vps_name = config.get("vps_name", "LOCAL")
master_name = f"[{vps_name} | COPY_BASE | {args.pair_id}]"
try:
    ctypes.windll.kernel32.SetConsoleTitleW(f"MASTER COPY_BASE {master_name}")
except Exception:
    pass
dan_tran_cua_so(4)
...
if saved_state_raw:
    ...
    print(f"[COPY_BASE] Khoi phuc so lenh: {len(lich_su_lenh)} lenh.")
else:
    huong_dang_danh = None
    lich_su_lenh = []
    thoi_diem_vao_lenh_cuoi = 0
    thoi_diem_vua_ra_lenh_dong = 0
    print("[COPY_BASE] Bat dau voi so lenh trong.")
...
def reload_runtime_config(new_cap):
    ...
    if old_exec_key != new_exec_key and len(lich_su_lenh) > 0:
        print("[HOT RELOAD] Dang co lenh COPY_BASE, khong doi execution broker/symbol.")
...
print(
    f"[COPY_BASE] Master san sang: execution={execution['exchange']}:{execution['symbol']} "
    f"role={execution['role']} volume={execution['volume']}"
)
logging.info("=== START MASTER COPY_BASE %s ===", args.pair_id)
...
try:
        ...
        try:
            ...
            if now_sec - last_config_check_time >= 1.0:
                ...
                if r.get(SHUTDOWN_KEY):
                    print("[SHUTDOWN] Master COPY_BASE nhan tin hieu tat.")
                ...
                if current_modified != last_config_modified:
                    ...
                    try:
                        ...
                        if new_cap:
                            ...
                            print(
                                f"[HOT RELOAD COPY_BASE] {dev_entry}|{dev_close}, pivot {spread_pivot:+.2f}, "
                                f"{stable_mode}, {stable_time_sec * 1000:.0f}ms, "
                                f"execution {execution['exchange']}:{execution['symbol']}"
                            )
                            logging.info(
                                "[HOT RELOAD COPY_BASE] entry=%s close=%s pivot=%s execution=%s:%s",
                                dev_entry,
                                dev_close,
                                spread_pivot,
                                execution["exchange"],
                        ...
                        print(f"[HOT RELOAD COPY_BASE] Loi config, giu cau hinh cu: {exc}")
                        logging.error("[HOT RELOAD COPY_BASE] Loi config: %s", exc)
            ...
            while True:
                ...
                try:
                    ...
                    luu_tri_nho()
                    print(f"[COPY_BASE] Da ghi so ticket #{ticket} {action} ({loai_lenh}) tu job {job_id}.")
                except Exception as exc:
                    print(f"[COPY_BASE] Loi doc ORDER_RESULT: {exc}")

            expired_jobs = [jid for jid, data in pending_jobs.items() if now_sec - data.get("time", now_sec) > 60]
            for jid in expired_jobs:
                print(f"[COPY_BASE] Xoa job mo lenh qua 60s khong co ticket: {jid}")
            ...
            if not trong_thoi_gian_bao_ve:
                ...
                for order_data in lich_su_lenh:
                    ...
                    print(f"[COPY_BASE] Ticket #{order_data['ticket']} bien mat. Lay lich su va ghi so.")
                    ...
                    for pos in sorted(untracked_positions, key=lambda x: x.get("time_msc", 0)):
                        ...
                        if loai_lenh_pos == "UNKNOWN":
                            ...
                            close_data = {
                                ...
                                "action_type": "COPY_BASE_CLOSE",
                                "conf_dev_close": dev_close,
                                "tick_hz_base_out": tick_base.get("tick_hz", 0),
                                "tick_hz_diff_out": tick_diff.get("tick_hz", 0),
                            }
                            gui_lenh_dong(order_data, close_data, "ORPHAN_UNKNOWN")
                            print(f"[COPY_BASE] Cat lenh khong ro huong #{pos['ticket']} bang ticket.")
                            thoi_diem_vua_ra_lenh_dong = time.time()
                            continue
                        ...
                        if huong_dang_danh and loai_lenh_pos != "UNKNOWN" and loai_lenh_pos != huong_dang_danh:
                            ...
                            print(f"[COPY_BASE] Cat lenh la nguoc huong #{pos['ticket']} {side}.")
                            thoi_diem_vua_ra_lenh_dong = time.time()
                        else:
                            adopted = {
                            ...
                            print(f"[COPY_BASE] Adopt ticket #{pos['ticket']} {side} ({loai_lenh_pos}) vao so lenh.")
                    luu_tri_nho()
            ...
            if not tick_base.get("connected", False) or not tick_diff.get("connected", False):
                print("[COPY_BASE] Mat mang/tick source (Base/Diff), khoa lenh.", end="\r")
                thoi_diem_nhan_tick_cuoi = time.time()
            ...
            if not tick_exec.get("connected", False):
                print("[COPY_BASE] Mat mang san thu 3 (Execution), khoa lenh.", end="\r")
                thoi_diem_nhan_tick_cuoi = time.time()
                da_xu_ly_lenh_cho_tick_nay = True
                continue
            ...
            if now - local_nhan_base > max_tick_delay or now - local_nhan_diff > max_tick_delay:
                print(f"[COPY_BASE] Tick Base/Diff tre qua {max_tick_delay}s, khoa lenh.", end="\r")
                da_xu_ly_lenh_cho_tick_nay = True
                continue

            # 🆕 Kiem tra do tre tick san thu 3
            if now - local_nhan_exec > max_tick_delay:
                print(f"[COPY_BASE] Tick san thu 3 tre qua {max_tick_delay}s, khoa lenh.", end="\r")
                da_xu_ly_lenh_cho_tick_nay = True
                continue

            if trong_gio_cam:
                if lich_su_lenh:
                    print(f"[COPY_BASE] Gio cam: xa {len(lich_su_lenh)} lenh COPY_BASE.")
                    for order_data in lich_su_lenh[:]:
                        close_data = {
                            "chenh_dong": 0,
                            "chenh_dong_raw": 0,
            ...
            if hanh_dong == "DONG_LENH" and lich_su_lenh:
                if tick_hz_vuot_nguong:
                    print(f"[COPY_BASE] Tick Hz vuot nguong, khoa chot. {hz_base}|{hz_diff}", end="\r")
                    continue
                ...
                close_data = {
                    ...
                    "mode_dong": mode_dong,
                    "action_type": tin_hieu.get("loai_dong", "COPY_BASE_CLOSE"),
                    "conf_dev_close": dev_close,
                    "tick_hz_base_out": tick_base.get("tick_hz", 0),
                    "tick_hz_diff_out": tick_diff.get("tick_hz", 0),
                }
                ...
                luu_tri_nho()
                print(
                    f"[COPY_BASE] Dong ticket #{order_data['ticket']} {close_action}, "
                    f"dev {close_data['chenh_dong']:.2f}."
                )

            elif hanh_dong == "VAO_LENH":
                if tick_hz_vuot_nguong:
                    print(f"[COPY_BASE] Tick Hz vuot nguong, khoa vao. {hz_base}|{hz_diff}", end="\r")
                    continue
                if equity_execution < alert_equity:
                    print(
                        f"[COPY_BASE] LOW EQUITY {execution['exchange']} {equity_execution:.2f} < {alert_equity}",
                        end="\r",
                    )
                    continue
                ...
                # 🆕 COPY_BASE: luon lay lenh theo chieu Diff
                ...
                luu_tri_nho()
                print(
                    f"[COPY_BASE] Vao {loai_lenh_moi} {action} {execution['exchange']}:{execution['symbol']} "
                    f"dev {tin_hieu['chenh_lech']:.2f} job {job_id}."
                )
                logging.info(
                    "[COPY_BASE ENTRY] %s %s dev=%s raw=%s job=%s",
                    loai_lenh_moi,
                    action,
                    tin_hieu["chenh_lech"],
            ...
            print(f"\n[COPY_BASE CRITICAL] {exc}")
            logging.error("[COPY_BASE CRITICAL]", exc_info=True)
            thoi_diem_nhan_tick_cuoi = time.time()
            da_xu_ly_lenh_cho_tick_nay = True
            time.sleep(0.5)
            continue
    ...
    print(f"\n[MASTER COPY_BASE {args.pair_id}] Tat an toan.")
    logging.info("=== STOP MASTER COPY_BASE ===")

print(f"[MASTER COPY_BASE {args.pair_id}] Da thoat.")
